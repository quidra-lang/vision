...
int | error run()
    ...
    tensor<uint16> cropped = try vision.crop(pixels, top = 0, left = 1, height = 2, width = 1)
    tensor<uint16> resized = try vision.resize(pixels, height = 4, width = 4)
    tensor<uint16> flipped = try vision.flip_horizontal(pixels)
